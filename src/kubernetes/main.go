...
func main() {
	...
	if client, err = getClientset(); err != nil {
	...
	fmt.Printf("Deployment finished with labels: %+v\n", deploymentLabels)
...
func deploy(ctx context.Context, client *kubernetes.Clientset,) (map[string]string, error) {
	...
	if err != nil && errors.IsNotFound(err){
		...
		if err != nil {	
			return nil, fmt.Errorf("deployment error: %v", err)
		}
...
func waitForPods(ctx context.Context, client *kubernetes.Clientset, deploymentlabels map[string]string) error {
	for {
		...
		if err != nil {
			return fmt.Errorf("error validating labels: %s", err)
		}

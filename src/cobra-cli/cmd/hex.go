...
securerandom hex -l 10`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		hexString, err := generateHexString(length)
		if err != nil {
			fmt.Println("Error generating hex string:", err)
			return
		}
		fmt.Println("Generated hex string:", hexString)
	},
}
...
func init() {
	...
	hexCmd.Flags().IntP("length", "l", 4, "Length of the hex number to generate")
	// Add any additional flags or configuration settings for the hex command here.
